...
namespace BillomatNet.Data
{
    ...
    public class BillomatArticle : BillomatNumberedObject<BillomatArticle>
    {
            ...
            string description = null, string currencyCode = null, int? unitId = null)
        {
            ...
            if (!string.IsNullOrEmpty(articleNumber)) { parameters.Add("article_number", articleNumber); }
            ...
            if (!string.IsNullOrEmpty(currencyCode)) { parameters.Add("currency_code", currencyCode); }
            if (unitId.HasValue) { parameters.Add("unit_id", unitId.Value.ToString(CultureInfo.InvariantCulture)); }
